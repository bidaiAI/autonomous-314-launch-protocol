...
一个自带**内盘**、完全开源、去平台化、让发射与流动性回归市场本身的 **EVM 原生** 链上发射协议。毕业前，交易直接发生在 launch 合约自身；毕业时，流动性自动切换到标准 V2 风格 DEX，而不是继续依赖某个平台作为交易入口。

**官方前端：** [https://auto314.cc](https://auto314.cc)
...
- **毕业是状态切换，不是平台后台动作**
  - 切换到 canonical V2 pair 是合约生命周期的一部分，不是管理员手工操作流程
- **毕业前刻意保持单市场路径**
  - 通过转账限制，把早期价格发现尽量约束在一个 canonical 市场里，减少旁路池和市场碎片化
- **模式家族是协议 primitive，不是前端选项卡**
  - `0314`、`b314`、`1314..9314`、`f314` 是协议层一等公民，而不是 UI 临时拼装出来的“套餐”
- **手续费结构偏向 creator，而不是偏向平台租金**
  - 毕业前手续费的结构性设计，是让项目侧拿到更大的经济面，而不是让平台先抽走最大头
- **前端可替换，入口不独占**
  - 官方前端只是参考实现，钱包、Bot、聚合器、白标站点都可以直接接协议，不需要经过官方许可
- **后台是轻层，不是交易引擎**
  - 执行在链上，服务端主要承担索引、图表、展示层职责，所以整体运行成本和复杂度低于平台自营交易引擎

一句话概括：

> **Autonomous 314 不是“带合约的网站型 launchpad”，而是一套由合约自身承载市场、储备逻辑与毕业路径的发射协议。**
...
- **协议集成总览：** [`docs/INTEGRATION.md`](docs/INTEGRATION.md)
- **metadata 结构：** [`docs/LAUNCH_METADATA.md`](docs/LAUNCH_METADATA.md)
...
- 白名单地址总数量必须 **大于等于席位数，且最多为席位数的 3 倍**
- 每个批准地址只能按**精确席位金额**承诺一次；其它任何金额都会直接被 revert 拒绝
...
- 经济模型更强调**回归市场**，让更多手续费留在项目侧，而不是被平台整体抽走
- 官方前端只是**参考实现**，不是唯一入口，也不是 gatekeeper

如果用一句话概括这个项目的立场，就是：

- **不是平台优先，而是协议优先**
- **不是平台抽成优先，而是让发射与流动性回归市场**
- **不是依赖入口生存，而是合约自身成体系**
...
- `b314` 与 `f314`：白名单窗口内直接向合约转原生币，表示固定席位 commit，但金额必须与席位金额完全相等
...
这个协议的设计目标之一，不是继续强化平台对链上流动性的抽取，而是把市场主导权重新交还给合约与参与者，让发射、交易和毕业更多回归市场本身。
...
当前实际使用的 bonding curve 形状、来源和缩放方式，见：
...
- **Support deployer 部署交易**：`0xe7bf7a28e85e222a5387ef4ae520262217ff65be75d023fa8534c497f4119d36` / `0xcd77bf0e98327158f16c1645a9c2f0bc8b6c5589862ecc8344ef831723856140` / `0x29e4824246581158804daa1d86aaba675f04f1e497166eac9961872f394bdd82` / `0x18bc4724f5275a3fc4fd2e4c476c9e8c1d140b5a4d3eceffbdec8e8d5e024260`
- **Factory 部署交易 / 区块**：`0xf77b68c21d31c51f0dbbffb5756f233c9a6718d49f549c262220d92a875afc06` @ `91258253`
- **Whitelist-taxed deployer 绑定交易 / 区块**：`0xa61eb496afb880fda09cb5f5915905cb72a912eb1f1dc78fd4c4908a460a4ab1` @ `91258260`
- **部署 salt**：`0xdda123f499c5f7f6d817abb3686e99857361bf6990923787dd4d8c5aef555afd`
- **创建费**：标准/税版 `0.01 BNB`，白名单/`f314` `0.03 BNB`
- **Graduation target**：`12 BNB`
- **Curve profile**：`virtualTokenReserve = 107,036,752`，`virtualQuoteReserve ≈ 4.60555128 BNB`
- **源码验证**：生产 indexer 会先补官方 factory/deployer 验证，并自动提交每个新创建 launch 到 Sourcify；这已经会把源码展示到 BscScan 页面，额外的 explorer API key 只作为补充验证通道。
...
如果你要把 Autonomous 314 接到别的产品里，建议按下面的角色路径阅读：

- **钱包 / 交易 Bot**
  - [docs/INTEGRATION.md](./docs/INTEGRATION.md) — 完整协议集成面
- **前端 / Indexer**
  - [docs/INTEGRATION.md](./docs/INTEGRATION.md) — 合约读取、状态路由、参考 API
- **metadata / 社交媒体字段解析**
  - [docs/LAUNCH_METADATA.md](./docs/LAUNCH_METADATA.md) — 图片与社交链接字段结构
...
可以。这正是它的核心设计目标之一。官方前端只是参考实现，不是必须经过的 gatekeeper。第三方直接接入时，建议从 [`docs/INTEGRATION.md`](./docs/INTEGRATION.md) 开始。

### 为什么 creator 分成比协议高？

因为这套系统的目标，是让发射与流动性回归市场本身。协议需要可持续收入，但不应默认平台天然应该吃掉大部分 launch 手续费。
