...
contract LaunchTokenWhitelistTaxed is LaunchTokenWhitelist {
    ...
    function launchSuffix() public pure override returns (string memory) {
        return "f314";
    }
    ...
    function _validateTaxConfig(
        ...
        if (launchModeId_ != MODE_WHITELIST_TAX_F314) revert InvalidTaxConfig();
        if (taxBps_ < 100 || taxBps_ > 900 || taxBps_ % 100 != 0) revert InvalidTaxConfig();
    ...
    function _isAuthorizedWhitelistTaxedFactoryDeployment(address factory_) private view returns (bool) {
